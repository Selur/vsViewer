...
TimeLineSlider::TimeLineSlider(QWidget *a_pParent)
  : QWidget(a_pParent), m_maxFrame(999), m_fps(0.0), m_currentFrame(0), m_pointerAtFrame(0), m_displayMode(Time), m_bigStep(10),
    m_sideMargin(6), m_bottomMargin(2), m_slideLineHeight(12), m_slideLineFrameWidth(1), m_slideLineTicksSpacing(1), m_shortTickHeight(2),
    m_mediumTickHeight(3), m_longTickHeight(4), m_tickTextSpacing(1), m_textHeight(6), m_topMargin(2), m_minimumTicksSpacing(4),
    m_sliderPressed(false), m_labelsFont("Digital Mini")
{
  ...
  m_colorRoleMap = {
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              {SlideLine, &m_slideLineColor},           {ActiveFrame, &m_activeFrameColor},
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              {InactiveFrame, &m_inactiveFrameColor},   {CurrentFramePointer, &m_currentFramePointerColor},
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              {SlidingPointer, &m_slidingPointerColor}, {Bookmark, &m_bookmarkColor},
  };
...
int TimeLineSlider::frame() const { return m_currentFrame; }
...
void TimeLineSlider::setFrame(int a_frame, bool a_refreshCache)
{
  ...
  if (m_currentFrame != m_pointerAtFrame) m_pointerAtFrame = m_currentFrame;

  if (m_currentFrame == oldCurrentFrame) return;
...
void TimeLineSlider::setFramesNumber(int a_framesNumber, bool a_refreshCache)
{
  ...
  if (m_currentFrame > m_maxFrame) setFrame(m_maxFrame, a_refreshCache);
  repaint();
}
...
TimeLineSlider::DisplayMode TimeLineSlider::displayMode() const { return m_displayMode; }
...
void TimeLineSlider::setBigStep(int a_bigStep) { m_bigStep = std::max(std::abs(a_bigStep), 1); }
...
void TimeLineSlider::setColor(ColorRole a_role, const QColor &a_color)
{
  ...
  if (it == m_colorRoleMap.end()) return;
...
std::set<int> TimeLineSlider::bookmarks() const { return m_bookmarks; }
...
{
  if (m_bookmarks.size() == 0) return a_frame;

  if (m_bookmarks.size() == 1) return *m_bookmarks.begin();

  std::set<int>::iterator next = m_bookmarks.upper_bound(a_frame);

  if (next == m_bookmarks.begin()) return *next;

  if (next == m_bookmarks.end()) return *m_bookmarks.rbegin();
  ...
  if (forwardDiff < backDiff) return *next;
...
{
  if (m_currentFrame < m_maxFrame) setFrame(m_currentFrame + 1, false);
}
...
{
  if (m_currentFrame > 0) setFrame(m_currentFrame - 1, false);
}

// END OF void TimeLineSlider::slotStepDown()
//==============================================================================

void TimeLineSlider::slotBigStepUp() { slotStepBy(m_bigStep); }

// END OF void TimeLineSlider::slotBigStepUp()
//==============================================================================

void TimeLineSlider::slotBigStepDown() { slotStepBy(-m_bigStep); }
...
void TimeLineSlider::slotBookmarkCurrentFrame() { addBookmark(m_currentFrame); }

// END OF void TimeLineSlider::slotBookmarkCurrentFrame()
//==============================================================================

void TimeLineSlider::slotUnbookmarkCurrentFrame() { removeBookmark(m_currentFrame); }
...
void TimeLineSlider::mouseReleaseEvent(QMouseEvent *a_pEvent)
{
  ...
  if (emitSignal) emit signalSliderReleased();
...
void TimeLineSlider::paintEvent(QPaintEvent *a_pEvent)
{
  ...
  if ((m_displayMode == Frames) || (m_fps == 0.0)) l_displayMode = Frames;
  ...
  if (ticksAtExactFrames) {
    ...
    while (unitsPerTick > minUnitsPerTick) unitsPerTick /= 10.0;

    // Do sane division for seconds and minutes.
    if ((l_displayMode == Time) && (minUnitsPerTick > 0.1)) {
      unitsPerTick = 0.2;                                       // 2 sec
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 2.5;  // 5 sec
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 2.0;  // 10 sec
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 2.0;  // 20 sec
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 1.5;  // 30 sec
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 2.0;  // 1 min
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 2.0;  // 2 min
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 2.5;  // 5 min
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 2.0;  // 10 min
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 2.0;  // 20 min
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 1.5;  // 30 min
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 2.0;  // 1 hour
    }

    while (unitsPerTick < minUnitsPerTick) {
      unitsPerTick *= 2.0;
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 1.25;
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 2.0;
      if (unitsPerTick < minUnitsPerTick) unitsPerTick *= 2.0;
    }
...
{
  if (a_frame > m_maxFrame) a_frame = m_maxFrame;
  int framePos = (int)((double)(slideLineInnerWidth() - 1) / (double)m_maxFrame * (double)a_frame);
...
int TimeLineSlider::posToFrame(int a_pos) const
{
  ...
  int l_frame = (int)std::round((double)m_maxFrame / (double)(slideLineInnerWidth() - 1) * (double)(a_pos - start));
  return l_frame;
}
...
{
  QMargins margins(m_slideLineFrameWidth, m_slideLineFrameWidth, m_slideLineFrameWidth, m_slideLineFrameWidth);
...
{
  int widgetHeight
    = m_bottomMargin + m_slideLineHeight + m_slideLineTicksSpacing + m_longTickHeight + m_tickTextSpacing + m_textHeight + m_topMargin;
