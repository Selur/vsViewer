...
void MainWindow::callMethod(const QString& typ, const QString& value, const QString &input)
{
  if (typ == "changeTo" && QFile::exists(value)) {
    ...
    if (loadScriptFromFile(value) && m_pVapourSynthScriptProcessor->initialize(m_ui.scriptEdit->text(), m_scriptFilePath)) {
      m_pVapourSynthScriptProcessor->finalize();
...
void MainWindow::loadStartUpScript()
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (option == "--preview-only" && i + 1 <= argc) {
      ...
      if (loadScriptFromFile(filePath) && m_pVapourSynthScriptProcessor->initialize(m_ui.scriptEdit->text(), m_scriptFilePath)) {
        m_pVapourSynthScriptProcessor->finalize();
        m_pPreviewDialog->previewScript(m_ui.scriptEdit->text(), m_scriptFilePath);
    ...
    if (m_ipcServer == nullptr && ipcID.isEmpty()) {
      if (option == "--listen" && i + 1 <= argc) {
        ...
        connect(m_ipcServer, SIGNAL(messageReceived(QString)), this, SLOT(ipcMessageReceived(QString)));
      }
      continue;
    }
