...
ScriptEditor::ScriptEditor(QWidget * a_pParent)
    : QPlainTextEdit(a_pParent), m_pSettingsManager(nullptr), m_pSideBox(nullptr),
        m_sideBoxLineWidth(1), m_sideBoxTextMargin(3), m_pCompleterModel(nullptr),
        m_pCompleter(nullptr), m_pSyntaxHighlighter(nullptr), m_typedCharacters(0),
        m_charactersTypedToStartCompletion(2), m_plainText(), m_backgroundColor(Qt::white),
        m_activeLineColor(Qt::lightGray), m_commonScriptTextFormat()
{
  ...
  connect(m_pCompleter, SIGNAL(activated(const QString &)), this,
      SLOT(slotInsertCompletion(const QString &)));
  connect(this, SIGNAL(textChanged()), this, SLOT(slotTextChanged()));
  connect(this, SIGNAL(blockCountChanged(int)), this, SLOT(slotUpdateSideBoxWidth()));
  connect(this, SIGNAL(updateRequest(const QRect &, int)), this,
      SLOT(slotUpdateSideBox(const QRect &, int)));
  connect(this, SIGNAL(cursorPositionChanged()), this, SLOT(slotHighlightCurrentBlock()));
...
{
  connect(a_pSettingsDialog, SIGNAL(signalSettingsChanged()), this, SLOT(slotLoadSettings()));
  connect(a_pSettingsDialog, SIGNAL(signalSettingsChanged()), m_pSyntaxHighlighter,
      SLOT(slotLoadSettings()));
}
...
void ScriptEditor::slotLoadSettings()
{
  ...
  m_commonScriptTextFormat = m_pSettingsManager->getTextFormat(TEXT_FORMAT_ID_COMMON_SCRIPT_TEXT);
...
void ScriptEditor::slotComplete()
{
  ...
  while ((wordStart > 0)
      && (lineString[wordStart - 1].isLetterOrNumber()
  ...
  if ((charactersAfterDot < m_charactersTypedToStartCompletion) && (pAction == nullptr)) {
  ...
  cursorRectangle.setWidth(
      m_pCompleter->popup()->sizeHintForColumn(0)
...
{
  QTextCursor currentCursor = textCursor();
  currentCursor.movePosition(QTextCursor::Left, QTextCursor::KeepAnchor, m_typedCharacters);
...
void ScriptEditor::resizeEvent(QResizeEvent * a_pEvent)
{
  ...
  m_pSideBox->setGeometry(QRect(cr.left(), cr.top(), sideBoxWidth(), cr.height()));
...
QString ScriptEditor::getVapourSynthCoreName()
{
  ...
  for (int k = 0; k < blocksCount; ++k) {
    QString simplifiedText = document()->findBlockByNumber(k).text().simplified();
    ...
    if (simplifiedText.mid(i, 4) == " as ") {
      ...
      for (; j < simplifiedText.length(); ++j) {
        if (!(simplifiedText[j].isLetterOrNumber() || (simplifiedText[j] == '_')))
          break;
      }
  ...
  for (int k = vsImportBlock + 1; k < blocksCount; ++k) {
    ...
    int j = i + 1;
    for (; i >= 0; --i) {
      if (!(simplifiedText[i].isLetterOrNumber() || (simplifiedText[i] == '_')))
        break;
    }
    i++;
...
void ScriptEditor::paintSideBox(QPaintEvent * a_pEvent)
{
  ...
  int lineNumberWidth = m_pSideBox->width() - m_sideBoxLineWidth - m_sideBoxTextMargin * 2;
  QPointF offset = contentOffset();
  int viewportHeight = viewport()->rect().height();
  qreal blockTop;
  for (QTextBlock textBlock = firstVisibleBlock(); textBlock.isValid() && ((blockTop =
      blockBoundingGeometry(textBlock).translated(offset).top()) < viewportHeight); textBlock =
      textBlock.next()) {
    if (!textBlock.isVisible())
      continue;
    QString number = QString::number(textBlock.blockNumber() + 1);
    painter.drawText(m_sideBoxTextMargin, blockTop, lineNumberWidth, labelHeight, Qt::AlignRight,
        number);
  }
}
...
void ScriptEditor::indentNewLine()
{
  ...
  QTextBlock previousBlock = document()->findBlockByNumber(blockNumber - 1);
