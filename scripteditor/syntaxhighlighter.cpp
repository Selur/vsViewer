...
{
  Undecided, Keyword, Operator, String, Number, Comment, VSCore, VSNamespace, VSFunction,
};
...
SyntaxHighlighter::SyntaxHighlighter(QTextDocument * a_pDocument, VSPluginsList a_pluginsList)
    : QSyntaxHighlighter(a_pDocument), m_pSettingsManager(nullptr), m_coreName("core"),
        m_pluginsList(a_pluginsList), m_keywordsList(), m_operatorsList(), m_keywordFormat(),
        m_operatorFormat(), m_stringFormat(), m_numberFormat(), m_commentFormat(), m_vsCoreFormat(),
        m_vsNamespaceFormat(), m_vsFunctionFormat(), m_vsArgumentFormat()
{
  m_keywordsList << "False" << "None" << "True" << "and" << "as" << "assert" << "break" << "class"
...
{
  m_keywordFormat = m_pSettingsManager->getTextFormat(TEXT_FORMAT_ID_KEYWORD);
  m_operatorFormat = m_pSettingsManager->getTextFormat(TEXT_FORMAT_ID_OPERATOR);
  m_stringFormat = m_pSettingsManager->getTextFormat(TEXT_FORMAT_ID_STRING);
  m_numberFormat = m_pSettingsManager->getTextFormat(TEXT_FORMAT_ID_NUMBER);
  m_commentFormat = m_pSettingsManager->getTextFormat(TEXT_FORMAT_ID_COMMENT);
  m_vsCoreFormat = m_pSettingsManager->getTextFormat(TEXT_FORMAT_ID_VS_CORE);
  m_vsNamespaceFormat = m_pSettingsManager->getTextFormat(TEXT_FORMAT_ID_VS_NAMESPACE);
  m_vsFunctionFormat = m_pSettingsManager->getTextFormat(TEXT_FORMAT_ID_VS_FUNCTION);
  m_vsArgumentFormat = m_pSettingsManager->getTextFormat(TEXT_FORMAT_ID_VS_ARGUMENT);
  rehighlight();
}
...
//------Long string continuation, single quotes---------------------------------
    ...
    if ((i == 0)
      ...
      for (j = i; j < textLength - 2; ++j) {
        if ((a_text.mid(j, 3) == "'''") && ((j == 0) || ((j != 0) && (a_text[j - 1] != '\\')))) {
...
//------Short string, single quotes---------------------------------------------
    ...
    if ((a_text[i] == '\'') || (a_text.mid(i, 2).toLower() == "r'")
      ...
      for (; j < textLength; ++j) {
        if ((a_text[j] == '\'') && ((j == 0) || ((j != 0) && (a_text[j - 1] != '\\'))))
          break;
      }
...
//------------------------------------------------------------------------------
  ...
  for (size_t k = 0; k < tokensNumber; ++k) {
    ...
    if (token.text == m_coreName)
    ...
    else if ((k > 1) && (tokens[k - 1].text == ".") && (tokens[k - 2].type == TokenType::VSCore)) {
      for (const VSData::Plugin & plugin : m_pluginsList) {
        ...
        && (tokens[k - 2].type == TokenType::VSNamespace) && (tokens[k - 3].text == ".")
