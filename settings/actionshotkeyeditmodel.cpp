...
ActionsHotkeyEditModel::ActionsHotkeyEditModel(SettingsManager * a_pSettingsManager,
    QObject * a_pParent)
...
QVariant ActionsHotkeyEditModel::data(const QModelIndex & a_index, int a_role) const
{
  if (!a_index.isValid())
    return QVariant();
  ...
  else if ((a_index.column() == 0) && ((a_role == Qt::DisplayRole) || (a_role == Qt::ToolTipRole)))
    return m_actionsList[a_index.row()].text;
  else if ((a_index.column() == 1)
      && ((a_role == Qt::DisplayRole) || (a_role == Qt::ToolTipRole) || (a_role == Qt::EditRole)))
...
bool ActionsHotkeyEditModel::setData(const QModelIndex & a_index, const QVariant & a_value,
    int a_role)
{
...
void ActionsHotkeyEditModel::reloadHotkeysSettings()
{
  ...
  emit dataChanged(createIndex(2, 0), createIndex(2, m_actionsList.size() - 1));
}
