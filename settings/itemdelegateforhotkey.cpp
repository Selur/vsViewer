...
ClearableKeySequenceEditor::ClearableKeySequenceEditor(QWidget * a_pParent)
    : QWidget(a_pParent), m_pKeySequenceEdit(nullptr), m_pClearKeySequenceButton(nullptr)
{
  ...
  connect(m_pKeySequenceEdit, SIGNAL(editingFinished()), this, SIGNAL(editingFinished()));

  m_pClearKeySequenceButton = new QToolButton(this);
  m_pClearKeySequenceButton->setToolTip(QObject::tr("Erase hotkey"));
  m_pClearKeySequenceButton->setIcon(QIcon(":erase.png"));
  pLayout->addWidget(m_pClearKeySequenceButton);

  connect(m_pClearKeySequenceButton, SIGNAL(clicked()), m_pKeySequenceEdit, SLOT(clear()));
}
...
void ClearableKeySequenceEditor::slotSetKeySequence(const QKeySequence & a_keySequence)
{
    ...
    const QStyleOptionViewItem & a_option, const QModelIndex & a_index) const
{
  (void) a_option;
  (void) a_index;
  ClearableKeySequenceEditor * pEditor = new ClearableKeySequenceEditor(a_pParent);
  connect(pEditor, SIGNAL(editingFinished()), this, SLOT(slotCommitAndCloseEditor()));
  return pEditor;
}

void ItemDelegateForHotkey::setEditorData(QWidget * a_pEditor, const QModelIndex & a_index) const
{
  ClearableKeySequenceEditor * pEditor = static_cast<ClearableKeySequenceEditor *>(a_pEditor);
...
void ItemDelegateForHotkey::setModelData(QWidget * a_pEditor, QAbstractItemModel * a_model,
...
{
  ClearableKeySequenceEditor * pEditor = qobject_cast<ClearableKeySequenceEditor *>(sender());
