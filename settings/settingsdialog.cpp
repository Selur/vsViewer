...
SettingsDialog::SettingsDialog(SettingsManager * a_pSettingsManager, QWidget * a_pParent)
    :
        QDialog(a_pParent,
            (Qt::WindowFlags) 0 | Qt::Dialog | Qt::CustomizeWindowHint | Qt::WindowTitleHint
                | Qt::WindowCloseButtonHint), m_pSettingsManager(a_pSettingsManager),
        m_pActionsHotkeyEditModel(nullptr), m_pItemDelegateForHotkey(nullptr),
        m_pThemeElementsModel(nullptr)
{
  ...
  connect(m_ui.addVSPluginsPathButton, SIGNAL(clicked()), this, SLOT(slotAddVSPluginsPath()));
  connect(m_ui.removeVSPluginsPathButton, SIGNAL(clicked()), this, SLOT(slotRemoveVSPluginsPath()));
  connect(m_ui.selectVSPluginsPathButton, SIGNAL(clicked()), this, SLOT(slotSelectVSPluginsPath()));

  connect(m_ui.addVSDocumentationPathButton, SIGNAL(clicked()), this,
      SLOT(slotAddVSDocumentationPath()));
  connect(m_ui.removeVSDocumentationPathButton, SIGNAL(clicked()), this,
      SLOT(slotRemoveVSDocumentationPath()));
  connect(m_ui.selectVSDocumentationPathButton, SIGNAL(clicked()), this,
      SLOT(slotSelectVSDocumentationPath()));

  connect(m_ui.themeElementsList, SIGNAL(clicked(const QModelIndex &)), this,
      SLOT(slotThemeElementSelected(const QModelIndex &)));
...
void SettingsDialog::slotCall()
{
  ...
  m_ui.vsPluginsPathsListWidget->addItems(m_pSettingsManager->getVapourSynthPluginsPaths());
...
void SettingsDialog::slotApply()
{
  ...
  int vsDocumentationPathsNumber = m_ui.vsDocumentationPathsListWidget->count();
  ...
  m_pSettingsManager->setVapourSynthDocumentationPaths(vapourSynthDocumentationPaths);
...
void SettingsDialog::slotAddVSPluginsPath()
{
  ...
  QListWidgetItem * pListItem = new QListWidgetItem(newPath, m_ui.vsPluginsPathsListWidget);
  pListItem->setToolTip(newPath);
}
...
{
  QListWidgetItem * pCurrentItem = m_ui.vsPluginsPathsListWidget->currentItem();
  if (pCurrentItem)
    delete pCurrentItem;
}
...
void SettingsDialog::slotAddVSDocumentationPath()
{
  ...
  QListWidgetItem * pListItem = new QListWidgetItem(newPath, m_ui.vsDocumentationPathsListWidget);
  pListItem->setToolTip(newPath);
}
...
{
  QListWidgetItem * pCurrentItem = m_ui.vsDocumentationPathsListWidget->currentItem();
  if (pCurrentItem)
    delete pCurrentItem;
}
