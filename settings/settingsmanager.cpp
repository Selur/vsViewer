...
const char VAPOURSYNTH_DOCUMENTATION_PATHS_KEY[] = "vapoursynth_documentation_paths";
...
const TimeLineSlider::DisplayMode DEFAULT_TIMELINE_MODE = TimeLineSlider::DisplayMode::Time;
...
const ResamplingFilter DEFAULT_CHROMA_RESAMPLING_FILTER = ResamplingFilter::Spline16;
...
const char ACTION_ID_SET_ZOOM_MODE_FIT_TO_FRAME[] = "set_zoom_mode_fit_to_frame";
const char ACTION_ID_SET_ZOOM_SCALE_MODE_NEAREST[] = "set_zoom_scale_mode_nearest";
const char ACTION_ID_SET_ZOOM_SCALE_MODE_BILINEAR[] = "set_zoom_scale_mode_bilinear";
const char ACTION_ID_TOGGLE_CROP_PANEL[] = "toggle_crop_panel";
const char ACTION_ID_PASTE_CROP_SNIPPET_INTO_SCRIPT[] = "paste_crop_snippet_into_script";
...
SettingsManager::SettingsManager(QObject* a_pParent)
    : QObject(a_pParent), m_settingsFilePath()
{
  ...
  if (portableMode)
    ...
    m_settingsFilePath = QStandardPaths::writableLocation(QStandardPaths::GenericConfigLocation)
        + SETTINGS_FILE_NAME;
  }
}
...
QVariant SettingsManager::valueInGroup(const QString & a_group, const QString & a_key,
...
bool SettingsManager::setValueInGroup(const QString & a_group, const QString & a_key,
...
QVariant SettingsManager::value(const QString & a_key, const QVariant & a_defaultValue) const
{
  return valueInGroup(COMMON_GROUP, a_key, a_defaultValue);
}

bool SettingsManager::setValue(const QString & a_key, const QVariant & a_value)
{
...
{
  QKeySequence hotkey = valueInGroup(HOTKEYS_GROUP, a_actionID).value<QKeySequence>();
  if (hotkey.isEmpty())
...
{
  QVariant colorValue = valueInGroup(THEME_GROUP, a_colorID, getDefaultColor(a_colorID));
...
{
  return value(MAIN_WINDOW_MAXIMIZED_KEY, DEFAULT_MAIN_WINDOW_MAXIMIZED).toBool();
}
...
{
  return value(AUTO_LOAD_LAST_SCRIPT_KEY, DEFAULT_AUTO_LOAD_LAST_SCRIPT).toBool();
}
...
{
  return value(PROMPT_TO_SAVE_CHANGES_KEY, DEFAULT_PROMPT_TO_SAVE_CHANGES).toBool();
}
...
bool SettingsManager::setVapourSynthLibraryPaths(const QStringList & a_pathsList)
{
...
bool SettingsManager::setVapourSynthPluginsPaths(const QStringList & a_pathsList)
{
...
{
  QStringList paths =
      value(VAPOURSYNTH_DOCUMENTATION_PATHS_KEY, DEFAULT_DOCUMENTATION_PATHS).toStringList();
  paths.removeDuplicates();
  return paths;
}

bool SettingsManager::setVapourSynthDocumentationPaths(const QStringList & a_pathsList)
{
...
bool SettingsManager::setYuvToRgbConversionMatrix(YuvToRgbConversionMatrix a_matrix)
{
...
{
  return (ChromaPlacement) value(CHROMA_PLACEMENT_KEY, (int) DEFAULT_CHROMA_PLACEMENT).toInt();
}
...
{
  return value(BICUBIC_FILTER_PARAMETER_B_KEY, DEFAULT_BICUBIC_FILTER_PARAMETER_B).toDouble();
}
...
{
  return value(BICUBIC_FILTER_PARAMETER_C_KEY, DEFAULT_BICUBIC_FILTER_PARAMETER_C).toDouble();
}
...
{
  return value(COLOR_PICKER_VISIBLE_KEY, DEFAULT_COLOR_PICKER_VISIBLE).toBool();
}
