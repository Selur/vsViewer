...
ThemeElementsModel::ThemeElementsModel(SettingsManager * a_pSettingsManager, QObject * a_pParent)
    : QAbstractItemModel(a_pParent), m_pSettingsManager(a_pSettingsManager)
{
...
QModelIndex ThemeElementsModel::index(int a_row, int a_column, const QModelIndex & a_parent) const
{
  (void) a_parent;
...
Qt::ItemFlags ThemeElementsModel::flags(const QModelIndex & a_index) const
{
  ...
  Qt::ItemFlags cellFlags = Qt::NoItemFlags | Qt::ItemIsEnabled | Qt::ItemIsSelectable;
...
QVariant ThemeElementsModel::data(const QModelIndex & a_index, int a_role) const
{
  ...
  if ((a_index.row() >= (int) m_themeElementsList.size()) || (a_index.column() >= 1))
    return QVariant();

  if (a_role == Qt::DecorationRole)
    return m_themeElementsList[a_index.row()].icon;
  else if ((a_role == Qt::DisplayRole) || (a_role == Qt::ToolTipRole))
...
bool ThemeElementsModel::setData(const QModelIndex & a_index, const QVariant & a_value, int a_role)
{
  (void) a_index;
  (void) a_value;
  (void) a_role;
...
void ThemeElementsModel::addThemeElement(const ThemeElementData & a_themeElementData)
{
...
void ThemeElementsModel::addTextCharFormat(const QString & a_id, const QString & a_text)
{
  ...
  newThemeElementData.textCharFormat = m_pSettingsManager->getTextFormat(a_id);
  addThemeElement(newThemeElementData);
}

//==============================================================================

void ThemeElementsModel::addColor(const QString & a_id, const QString & a_text)
{
  ThemeElementData newThemeElementData;
...
void ThemeElementsModel::reloadThemeSettings()
{
  for (size_t i = 0; i < m_themeElementsList.size(); ++i) {
    if (m_themeElementsList[i].type == ThemeElementType::TextCharFormat) {
      m_themeElementsList[i].textCharFormat = m_pSettingsManager->getTextFormat(
          m_themeElementsList[i].id);
    } else if (m_themeElementsList[i].type == ThemeElementType::Color) {
      m_themeElementsList[i].color = m_pSettingsManager->getColor(m_themeElementsList[i].id);
    }
  }
}
...
bool ThemeElementsModel::saveThemeElementData(const ThemeElementData & a_themeElementData)
{
  for (size_t i = 0; i < m_themeElementsList.size(); ++i) {
    if (m_themeElementsList[i].id != a_themeElementData.id)
      continue;

    m_themeElementsList[i].textCharFormat = a_themeElementData.textCharFormat;
...
void ThemeElementsModel::slotSaveThemeSettings()
{
  for (size_t i = 0; i < m_themeElementsList.size(); ++i) {
    if (m_themeElementsList[i].type == ThemeElementType::TextCharFormat) {
      ...
      m_pSettingsManager->setColor(m_themeElementsList[i].id, m_themeElementsList[i].color);
    }
  }
}
