...
ScrollNavigator::ScrollNavigator(QWidget * a_pParent)
    : QWidget(a_pParent), m_contentsHeight(0), m_contentsWidth(0), m_viewportX(0), m_viewportY(0),
...
void ScrollNavigator::draw(int a_contentsWidth, int a_contentsHeight, int a_viewportX,
...
{
  if ((m_contentsWidth == 0) || (m_contentsHeight == 0) || (m_viewportWidth == 0)
      || (m_viewportHeight == 0) || (m_viewportX >= m_contentsWidth)
      || (m_viewportY >= m_contentsHeight)) {
    a_pPaintEvent->ignore();
    return;
  }

  int measures[] = { m_contentsWidth, m_contentsHeight, m_viewportWidth, m_viewportHeight };
  int maxMeasure = 0;
  for (int m : measures)
    if (m > maxMeasure)
      maxMeasure = m;

  int normalizedContentsWidth = int((double) m_contentsWidth * 100.0 / (double) maxMeasure);
  int normalizedContentsHeight = int((double) m_contentsHeight * 100.0 / (double) maxMeasure);
  int normalizedVieportX = int((double) m_viewportX * 100.0 / (double) maxMeasure);
  int normalizedViwportY = int((double) m_viewportY * 100.0 / (double) maxMeasure);
  int normalizedViewportWidth = int((double) m_viewportWidth * 100.0 / (double) maxMeasure);
  int normalizedViewportHeight = int((double) m_viewportHeight * 100.0 / (double) maxMeasure);
