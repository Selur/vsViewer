...
PreviewDialog::PreviewDialog(VapourSynthScriptProcessor * a_pVapourSynthScriptProcessor,
    SettingsManager * a_pSettingsManager, SettingsDialog * a_pSettingsDialog, QWidget * a_pParent)
    :
        QDialog(a_pParent,
            (Qt::WindowFlags) 0 | Qt::Window | Qt::CustomizeWindowHint
                | Qt::WindowMinimizeButtonHint | Qt::WindowMaximizeButtonHint
                | Qt::WindowCloseButtonHint),
        m_pVapourSynthScriptProcessor(a_pVapourSynthScriptProcessor),
        m_pSettingsManager(a_pSettingsManager), m_pSettingsDialog(a_pSettingsDialog),
        m_pAdvancedSettingsDialog(nullptr), m_pStatusBar(nullptr), m_currentFrame(0),
        m_bigFrameStep(10), m_scriptName(), m_framePixmap(), m_cpVideoInfo(nullptr),
        m_changingCropValues(false), m_pPreviewContextMenu(nullptr),
        m_pActionFrameToClipboard(nullptr), m_pActionSaveSnapshot(nullptr),
        m_pActionToggleZoomPanel(nullptr), m_pMenuZoomModes(nullptr),
        m_pActionGroupZoomModes(nullptr), m_pActionSetZoomModeNoZoom(nullptr),
        m_pActionSetZoomModeFixedRatio(nullptr), m_pActionSetZoomModeFitToFrame(nullptr),
        m_pMenuZoomScaleModes(nullptr), m_pActionGroupZoomScaleModes(nullptr),
        m_pActionSetZoomScaleModeNearest(nullptr), m_pActionSetZoomScaleModeBilinear(nullptr),
        m_pActionToggleCropPanel(nullptr), m_pActionToggleTimeLinePanel(nullptr),
        m_pMenuTimeLineModes(nullptr), m_pActionGroupTimeLineModes(nullptr),
        m_pActionSetTimeLineModeTime(nullptr), m_pActionSetTimeLineModeFrames(nullptr),
        m_pActionTimeStepForward(nullptr), m_pActionTimeStepBack(nullptr),
        m_pActionPasteCropSnippetIntoScript(nullptr), m_pActionAdvancedSettingsDialog(nullptr),
        m_pActionToggleColorPicker(nullptr), m_actionIDToZoomMode(), m_actionIDToZoomScaleMode(),
        m_actionIDToTimeLineMode(), m_settableActionsList()
{
  m_ui.setupUi(this);
  ...
  m_ui.colorPickerLabel->setVisible(m_pSettingsManager->getColorPickerVisible());
  ...
  connect(m_pAdvancedSettingsDialog, SIGNAL(signalSettingsChanged()), m_pVapourSynthScriptProcessor,
      SLOT(slotSettingsChanged()));
  connect(m_pAdvancedSettingsDialog, SIGNAL(signalSettingsChanged()), this,
      SLOT(slotAdvancedSettingsChanged()));
  connect(m_ui.frameNumberSlider, SIGNAL(signalFrameChanged(int)), this, SLOT(slotShowFrame(int)));
  ...
  connect(m_ui.previewArea, SIGNAL(signalCtrlWheel(QPoint)), this,
      SLOT(slotPreviewAreaCtrlWheel(QPoint)));
  connect(m_ui.previewArea, SIGNAL(signalMouseMiddleButtonReleased()), this,
      SLOT(slotPreviewAreaMouseMiddleButtonReleased()));
  connect(m_ui.previewArea, SIGNAL(signalMouseRightButtonReleased()), this,
      SLOT(slotPreviewAreaMouseRightButtonReleased()));
  connect(m_ui.previewArea, SIGNAL(signalMouseRightButtonReleased()), this,
      SLOT(slotPreviewAreaMouseRightButtonReleased()));
  connect(m_ui.previewArea, SIGNAL(signalMouseOverPoint(float, float)), this,
      SLOT(slotPreviewAreaMouseOverPoint(float, float)));
...
void PreviewDialog::previewScript(const QString& a_script, const QString& a_scriptName)
{
  ...
  bool initialized = m_pVapourSynthScriptProcessor->initialize(a_script, a_scriptName);
  if (!initialized) {
    hide();
    return;
  }
  ...
  if (m_cpVideoInfo->fpsDen == 0)
    ...
    m_ui.frameNumberSlider->setFPS((double) m_cpVideoInfo->fpsNum / (double) m_cpVideoInfo->fpsDen);
  }
...
void PreviewDialog::slotSaveSnapshot()
{
  ...
  snapshotFilePath = QFileDialog::getSaveFileName(this, QObject::tr("Save frame as image"),
      snapshotFilePath, QObject::tr("PNG image (*.png);;All files (*)"));

  if (!snapshotFilePath.isEmpty()) {
    bool success = m_framePixmap.save(snapshotFilePath, "PNG");
    if (!success) {
      QMessageBox::critical(this, QObject::tr("Image save error"),
          QObject::tr("Error while saving image ") + snapshotFilePath);
    }
  }
}
...
void PreviewDialog::slotZoomModeChanged()
{
  ...
  if (pSender == m_ui.zoomModeComboBox) {
    ...
    for (QAction * pAction : m_pActionGroupZoomModes->actions()) {
      ZoomMode actionZoomMode = m_actionIDToZoomMode[pAction->data().toString()];
...
void PreviewDialog::slotScaleModeChanged()
{
  ...
  Qt::TransformationMode scaleMode =
      (Qt::TransformationMode) m_ui.scaleModeComboBox->currentData().toInt();

  QObject * pSender = sender();
  if (pSender == m_ui.scaleModeComboBox) {
    for (QAction * pAction : m_pActionGroupZoomScaleModes->actions()) {
      Qt::TransformationMode actionScaleMode = m_actionIDToZoomScaleMode[pAction->data().toString()];
    ...
    // If signal wasn't sent by combo box - presume it was sent by action.
    QAction * pSenderAction = qobject_cast<QAction *>(pSender);
    scaleMode = m_actionIDToZoomScaleMode[pSenderAction->data().toString()];
...
void PreviewDialog::slotTimeLineModeChanged()
{
  ...
  TimeLineSlider::DisplayMode timeLineMode =
      (TimeLineSlider::DisplayMode) m_ui.timeLineModeComboBox->currentData().toInt();
  ...
  if (pSender == m_ui.timeLineModeComboBox) {
    ...
    // If signal wasn't sent by combo box - presume it was sent by action.
    QAction * pSenderAction = qobject_cast<QAction *>(pSender);
    timeLineMode = m_actionIDToTimeLineMode[pSenderAction->data().toString()];
...
void PreviewDialog::slotPreviewAreaMouseOverPoint(float a_normX, float a_normY)
{
  ...
  m_pVapourSynthScriptProcessor->colorAtPoint(frameX, frameY, value1, value2, value3);
  ...
  QString colorString =
      QString("%1:%2|%3:%4|%5:%6").arg(l1).arg(value1).arg(l2).arg(value2).arg(l3).arg(value3);
  m_ui.colorPickerLabel->setText(colorString);
}
...
//------------------------------------------------------------------------------
  ...
  m_pActionFrameToClipboard->setIconText(QObject::tr("Copy frame to clipboard"));
...
//------------------------------------------------------------------------------
  ...
  m_pActionSaveSnapshot->setIconText(QObject::tr("Save snapshot"));
...
//------------------------------------------------------------------------------
  ...
  m_pActionToggleZoomPanel->setIconText(QObject::tr("Show zoom panel"));
...
//------------------------------------------------------------------------------
  ...
  m_pMenuZoomModes->setTitle(QObject::tr("Zoom mode"));
...
//------------------------------------------------------------------------------
  ...
  m_pActionSetZoomModeNoZoom->setIconText(QObject::tr("Zoom: No zoom"));
...
//------------------------------------------------------------------------------
  ...
  m_pActionSetZoomModeFixedRatio->setIconText(QObject::tr("Zoom: Fixed ratio"));
  ...
  hotkey = m_pSettingsManager->getHotkey(ACTION_ID_SET_ZOOM_MODE_FIXED_RATIO);
  m_pActionSetZoomModeFixedRatio->setShortcut(hotkey);
  m_pActionSetZoomModeFixedRatio->setData(ACTION_ID_SET_ZOOM_MODE_FIXED_RATIO);
  m_settableActionsList.push_back(m_pActionSetZoomModeFixedRatio);
  m_actionIDToZoomMode[ACTION_ID_SET_ZOOM_MODE_FIXED_RATIO] = ZoomMode::FixedRatio;
  addAction(m_pActionSetZoomModeFixedRatio);

//------------------------------------------------------------------------------

  m_pActionSetZoomModeFitToFrame = new QAction(this);
  m_pActionSetZoomModeFitToFrame->setIconText(QObject::tr("Zoom: Fit to frame"));
  ...
  hotkey = m_pSettingsManager->getHotkey(ACTION_ID_SET_ZOOM_MODE_FIT_TO_FRAME);
  m_pActionSetZoomModeFitToFrame->setShortcut(hotkey);
  m_pActionSetZoomModeFitToFrame->setData(ACTION_ID_SET_ZOOM_MODE_FIT_TO_FRAME);
  m_settableActionsList.push_back(m_pActionSetZoomModeFitToFrame);
  m_actionIDToZoomMode[ACTION_ID_SET_ZOOM_MODE_FIT_TO_FRAME] = ZoomMode::FitToFrame;
...
//------------------------------------------------------------------------------
  ...
  m_pMenuZoomScaleModes->setTitle(QObject::tr("Zoom scale mode"));
...
//------------------------------------------------------------------------------
  ...
  m_pActionSetZoomScaleModeNearest->setIconText(QObject::tr("Scale: Nearest"));
  m_pActionSetZoomScaleModeNearest->setCheckable(true);
  m_pActionSetZoomScaleModeNearest->setActionGroup(m_pActionGroupZoomScaleModes);
  m_pMenuZoomScaleModes->addAction(m_pActionSetZoomScaleModeNearest);
  hotkey = m_pSettingsManager->getHotkey(ACTION_ID_SET_ZOOM_SCALE_MODE_NEAREST);
  m_pActionSetZoomScaleModeNearest->setShortcut(hotkey);
  m_pActionSetZoomScaleModeNearest->setData(ACTION_ID_SET_ZOOM_SCALE_MODE_NEAREST);
  m_settableActionsList.push_back(m_pActionSetZoomScaleModeNearest);
  m_actionIDToZoomScaleMode[ACTION_ID_SET_ZOOM_SCALE_MODE_NEAREST] = Qt::FastTransformation;
  addAction(m_pActionSetZoomScaleModeNearest);

//------------------------------------------------------------------------------

  m_pActionSetZoomScaleModeBilinear = new QAction(this);
  m_pActionSetZoomScaleModeBilinear->setIconText(QObject::tr("Scale: Bilinear"));
  m_pActionSetZoomScaleModeBilinear->setCheckable(true);
  m_pActionSetZoomScaleModeBilinear->setActionGroup(m_pActionGroupZoomScaleModes);
  m_pMenuZoomScaleModes->addAction(m_pActionSetZoomScaleModeBilinear);
  hotkey = m_pSettingsManager->getHotkey(ACTION_ID_SET_ZOOM_SCALE_MODE_BILINEAR);
  m_pActionSetZoomScaleModeBilinear->setShortcut(hotkey);
  m_pActionSetZoomScaleModeBilinear->setData(ACTION_ID_SET_ZOOM_SCALE_MODE_BILINEAR);
  m_settableActionsList.push_back(m_pActionSetZoomScaleModeBilinear);
  m_actionIDToZoomScaleMode[ACTION_ID_SET_ZOOM_SCALE_MODE_BILINEAR] = Qt::SmoothTransformation;
...
//------------------------------------------------------------------------------
  ...
  m_pActionToggleCropPanel->setIconText(QObject::tr("Crop assistant"));
...
//------------------------------------------------------------------------------
  ...
  m_pActionToggleTimeLinePanel->setIconText(QObject::tr("Show timeline panel"));
...
//------------------------------------------------------------------------------
  ...
  m_pMenuTimeLineModes->setTitle(QObject::tr("Timeline display mode"));
...
//------------------------------------------------------------------------------
  ...
  m_pActionSetTimeLineModeTime->setIconText(QObject::tr("Timeline mode: Time"));
  m_pActionSetTimeLineModeTime->setIcon(QIcon(":timeline.png"));
  m_pActionSetTimeLineModeTime->setCheckable(true);
  m_pActionSetTimeLineModeTime->setActionGroup(m_pActionGroupTimeLineModes);
  m_pMenuTimeLineModes->addAction(m_pActionSetTimeLineModeTime);
  hotkey = m_pSettingsManager->getHotkey(ACTION_ID_SET_TIMELINE_MODE_TIME);
  m_pActionSetTimeLineModeTime->setShortcut(hotkey);
  m_pActionSetTimeLineModeTime->setData(ACTION_ID_SET_TIMELINE_MODE_TIME);
  m_settableActionsList.push_back(m_pActionSetTimeLineModeTime);
  m_actionIDToTimeLineMode[ACTION_ID_SET_TIMELINE_MODE_TIME] = TimeLineSlider::DisplayMode::Time;
  addAction(m_pActionSetTimeLineModeTime);

//------------------------------------------------------------------------------

  m_pActionSetTimeLineModeFrames = new QAction(this);
  m_pActionSetTimeLineModeFrames->setIconText(QObject::tr("Timeline mode: Frames"));
  m_pActionSetTimeLineModeFrames->setIcon(QIcon(":timeline_frames.png"));
  m_pActionSetTimeLineModeFrames->setCheckable(true);
  m_pActionSetTimeLineModeFrames->setActionGroup(m_pActionGroupTimeLineModes);
  m_pMenuTimeLineModes->addAction(m_pActionSetTimeLineModeFrames);
  hotkey = m_pSettingsManager->getHotkey(ACTION_ID_SET_TIMELINE_MODE_FRAMES);
  m_pActionSetTimeLineModeFrames->setShortcut(hotkey);
  m_pActionSetTimeLineModeFrames->setData(ACTION_ID_SET_TIMELINE_MODE_FRAMES);
...
//------------------------------------------------------------------------------
  ...
  m_pActionTimeStepForward->setIconText(QObject::tr("Time: step forward"));
...
//------------------------------------------------------------------------------
  ...
  m_pActionTimeStepBack->setIconText(QObject::tr("Time: step back"));
...
//------------------------------------------------------------------------------
  ...
  m_pActionPasteCropSnippetIntoScript->setIconText(QObject::tr("Paste crop snippet into script"));
  m_pActionPasteCropSnippetIntoScript->setIcon(QIcon(":paste.png"));
  hotkey = m_pSettingsManager->getHotkey(ACTION_ID_PASTE_CROP_SNIPPET_INTO_SCRIPT);
  m_pActionPasteCropSnippetIntoScript->setShortcut(hotkey);
  m_pActionPasteCropSnippetIntoScript->setData(ACTION_ID_PASTE_CROP_SNIPPET_INTO_SCRIPT);
  m_settableActionsList.push_back(m_pActionPasteCropSnippetIntoScript);

//------------------------------------------------------------------------------

  m_pActionAdvancedSettingsDialog = new QAction(this);
  m_pActionAdvancedSettingsDialog->setIconText(QObject::tr("Preview advanced settings"));
  m_pActionAdvancedSettingsDialog->setIcon(QIcon(":settings.png"));
  hotkey = m_pSettingsManager->getHotkey(ACTION_ID_ADVANCED_PREVIEW_SETTINGS);
  m_pActionAdvancedSettingsDialog->setShortcut(hotkey);
  m_pActionAdvancedSettingsDialog->setData(ACTION_ID_ADVANCED_PREVIEW_SETTINGS);
  m_settableActionsList.push_back(m_pActionAdvancedSettingsDialog);

//------------------------------------------------------------------------------

  m_pActionToggleColorPicker = new QAction(this);
  m_pActionToggleColorPicker->setIconText(QObject::tr("Color panel"));
...
//------------------------------------------------------------------------------
  ...
  connect(m_pActionFrameToClipboard, SIGNAL(triggered()), this, SLOT(slotFrameToClipboard()));
  connect(m_pActionSaveSnapshot, SIGNAL(triggered()), this, SLOT(slotSaveSnapshot()));
  connect(m_pActionToggleZoomPanel, SIGNAL(toggled(bool)), this,
      SLOT(slotToggleZoomPanelVisible(bool)));
  connect(m_pActionSetZoomModeNoZoom, SIGNAL(toggled(bool)), this, SLOT(slotZoomModeChanged()));
  connect(m_pActionSetZoomModeFixedRatio, SIGNAL(toggled(bool)), this, SLOT(slotZoomModeChanged()));
  connect(m_pActionSetZoomModeFitToFrame, SIGNAL(toggled(bool)), this, SLOT(slotZoomModeChanged()));
  connect(m_pActionSetZoomScaleModeNearest, SIGNAL(toggled(bool)), this,
      SLOT(slotScaleModeChanged()));
  connect(m_pActionSetZoomScaleModeBilinear, SIGNAL(toggled(bool)), this,
      SLOT(slotScaleModeChanged()));
  connect(m_pActionToggleCropPanel, SIGNAL(toggled(bool)), this,
      SLOT(slotToggleCropPanelVisible(bool)));
  connect(m_pActionToggleTimeLinePanel, SIGNAL(toggled(bool)), this,
      SLOT(slotToggleTimeLinePanelVisible(bool)));
  connect(m_pActionSetTimeLineModeTime, SIGNAL(toggled(bool)), this,
      SLOT(slotTimeLineModeChanged()));
  connect(m_pActionSetTimeLineModeFrames, SIGNAL(toggled(bool)), this,
      SLOT(slotTimeLineModeChanged()));
  connect(m_pActionTimeStepForward, SIGNAL(triggered()), this, SLOT(slotTimeStepForward()));
  connect(m_pActionTimeStepBack, SIGNAL(triggered()), this, SLOT(slotTimeStepBack()));
  connect(m_pActionPasteCropSnippetIntoScript, SIGNAL(triggered()), this,
      SLOT(slotPasteCropSnippetIntoScript()));
  connect(m_pActionAdvancedSettingsDialog, SIGNAL(triggered()), m_pAdvancedSettingsDialog,
      SLOT(slotCall()));
  connect(m_pActionToggleColorPicker, SIGNAL(toggled(bool)), this,
      SLOT(slotToggleColorPicker(bool)));
...
void PreviewDialog::setUpZoomPanel()
{
  ...
  m_ui.zoomModeComboBox->addItem(QIcon(":zoom_no_zoom.png"), QObject::tr("No zoom"),
      (int) ZoomMode::NoZoom);
  m_ui.zoomModeComboBox->addItem(QIcon(":zoom_fixed_ratio.png"), QObject::tr("Fixed ratio"),
      (int) ZoomMode::FixedRatio);
  m_ui.zoomModeComboBox->addItem(QIcon(":zoom_fit_to_frame.png"), QObject::tr("Fit to frame"),
      (int) ZoomMode::FitToFrame);
  ...
  m_ui.scaleModeComboBox->addItem(QObject::tr("Nearest"), (int) Qt::FastTransformation);
  m_ui.scaleModeComboBox->addItem(QObject::tr("Bilinear"), (int) Qt::SmoothTransformation);
  bool noZoom = (zoomMode == ZoomMode::NoZoom);
  m_ui.scaleModeComboBox->setEnabled(!noZoom);
  ...
  connect(m_ui.zoomModeComboBox, SIGNAL(currentIndexChanged(int)), this,
      SLOT(slotZoomModeChanged()));
  connect(m_ui.zoomRatioSpinBox, SIGNAL(valueChanged(double)), this,
      SLOT(slotZoomRatioChanged(double)));
  connect(m_ui.scaleModeComboBox, SIGNAL(currentIndexChanged(int)), this,
      SLOT(slotScaleModeChanged()));
}
...
void PreviewDialog::setUpTimeLinePanel()
{
  ...
  m_ui.timeLineModeComboBox->addItem(QIcon(":timeline.png"), QObject::tr("Time"),
      (int) TimeLineSlider::DisplayMode::Time);
  m_ui.timeLineModeComboBox->addItem(QIcon(":timeline_frames.png"), QObject::tr("Frames"),
      (int) TimeLineSlider::DisplayMode::Frames);
  ...
  connect(m_ui.timeStepEdit, SIGNAL(timeChanged(const QTime &)), this,
      SLOT(slotTimeStepChanged(const QTime &)));
  connect(m_ui.timeLineModeComboBox, SIGNAL(currentIndexChanged(int)), this,
      SLOT(slotTimeLineModeChanged()));
}
...
void PreviewDialog::setUpCropPanel()
{
  ...
  m_ui.cropModeComboBox->addItem(QObject::tr("Absolute"), (int) CropMode::Absolute);
  m_ui.cropModeComboBox->addItem(QObject::tr("Relative"), (int) CropMode::Relative);
  ...
  connect(m_ui.cropModeComboBox, SIGNAL(currentIndexChanged(int)), this,
      SLOT(slotCropModeChanged()));
...
void PreviewDialog::setPreviewPixmap()
{
  if (m_ui.cropPanel->isVisible()) {
    ...
    QPixmap zoomedPixmap = croppedPixmap.scaled(croppedPixmap.width() * ratio,
        croppedPixmap.height() * ratio, Qt::KeepAspectRatio, Qt::FastTransformation);
