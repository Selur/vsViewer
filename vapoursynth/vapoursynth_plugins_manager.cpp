...
VapourSynthPluginsManager::VapourSynthPluginsManager(SettingsManager *a_pSettingsManager, const VSAPI *a_cpVSAPI, QObject *a_pParent)
  : QObject(a_pParent), m_pluginsList(), m_currentPluginPath(), m_pluginAlreadyLoaded(false), m_pSettingsManager(a_pSettingsManager)
{
  if (a_pParent) {
    connect(this, SIGNAL(signalWriteLogMessage(int, const QString &)), a_pParent, SLOT(slotWriteLogMessage(int, const QString &)));
  }
...
VapourSynthPluginsManager::~VapourSynthPluginsManager() { slotClear(); }
...
void VapourSynthPluginsManager::getCorePlugins(const VSAPI *a_cpVSAPI)
{
  ...
  while (pPlugin) {
    ...
    VSPluginFunction *pPluginFunction = a_cpVSAPI->getNextPluginFunction(nullptr, pPlugin);
    while (pPluginFunction) {
      const char *pluginFunctionName = a_cpVSAPI->getPluginFunctionName(pPluginFunction);
      const char *pluginFunctionArgs = a_cpVSAPI->getPluginFunctionArguments(pPluginFunction);

      VSData::Function function = parseFunctionSignature(pluginFunctionName, pluginFunctionArgs);
...
QStringList VapourSynthPluginsManager::functions() const
{
  ...
  for (const VSData::Plugin &plugin : m_pluginsList) {
    ...
    for (const VSData::Function &function : plugin.functions) functionsList << functionNamespace + function.toString();
  }
...
VSPluginsList VapourSynthPluginsManager::pluginsList() const { return m_pluginsList; }

// END OF VSPluginsList VapourSynthPluginsManager::pluginsList() const
//==============================================================================

VSData::Function VapourSynthPluginsManager::parseFunctionSignature(const QString &a_name, const QString &a_arguments)
{
  VSData::Function function;
  function.name = a_name;
  QStringList argumentsList = a_arguments.split(';', Qt::SkipEmptyParts);
  if (argumentsList.size() == 0) return function;

  // This is true for arguments lists returned by VSAPI.
  if (argumentsList[0] == a_name) argumentsList.removeFirst();
...
void VapourSynthPluginsManager::slotClear() { m_pluginsList.clear(); }
...
void VapourSynthPluginsManager::slotSort()
{
  ...
  for (VSData::Plugin &plugin : m_pluginsList) std::stable_sort(plugin.functions.begin(), plugin.functions.end());
}
