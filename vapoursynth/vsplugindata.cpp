...
VSData::FunctionArgument::FunctionArgument()
    : name(), type(), optional(false), empty(false)
{

}

VSData::FunctionArgument::FunctionArgument(const VSData::FunctionArgument & a_other)
    : name(a_other.name), type(a_other.type), optional(a_other.optional), empty(a_other.empty)
{

}

VSData::FunctionArgument::FunctionArgument(VSData::FunctionArgument && a_other)
    : name(std::move(a_other.name)), type(std::move(a_other.type)),
        optional(std::move(a_other.optional)), empty(std::move(a_other.empty))
{

}

VSData::FunctionArgument & VSData::FunctionArgument::operator=(VSData::FunctionArgument a_other)
{
...
bool VSData::FunctionArgument::operator<(const VSData::FunctionArgument & a_other) const
{
...
VSData::Function::Function(const VSData::Function & a_other)
    : name(a_other.name), arguments(a_other.arguments)
{

}

VSData::Function::Function(VSData::Function && a_other)
    : name(std::move(a_other.name)), arguments(std::move(a_other.arguments))
{
...
QString VSData::Function::toString() const
{
  ...
  for (firstOptional = 0;
      (firstOptional < argumentsNumber) && (!argumentsCopy[firstOptional].optional);
      ++firstOptional) {
  };
...
VSData::Plugin::Plugin()
    : filepath(), id(), pluginNamespace(), name(), functions()
{

}

VSData::Plugin::Plugin(const VSData::Plugin & a_other)
    : filepath(a_other.filepath), id(a_other.id), pluginNamespace(a_other.pluginNamespace),
        name(a_other.name), functions(a_other.functions)
{

}

VSData::Plugin::Plugin(VSData::Plugin && a_other)
    : filepath(std::move(a_other.filepath)), id(std::move(a_other.id)),
        pluginNamespace(std::move(a_other.pluginNamespace)), name(std::move(a_other.name)),
...
bool VSData::Plugin::operator<(const VSData::Plugin & a_other) const
{
  return (pluginNamespace.compare(a_other.pluginNamespace, Qt::CaseInsensitive) < 0);
}
