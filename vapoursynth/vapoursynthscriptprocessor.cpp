...
VapourSynthScriptProcessor::VapourSynthScriptProcessor(SettingsManager * a_pSettingsManager,
    QObject * a_pParent)
    : QObject(a_pParent), m_pSettingsManager(a_pSettingsManager), m_script(), m_scriptName(),
...
bool VapourSynthScriptProcessor::requestFrame(int a_frameNumber)
{
  ...
  const VSFrameRef * cpNewFrameRef = m_cpVSAPI->getFrame(a_frameNumber, m_pOutputNode,
...
QPixmap VapourSynthScriptProcessor::pixmap(int a_frameNumber)
{
  ...
  const VSFrameRef * cpFrameRef = m_cpVSAPI->getFrame(a_frameNumber, m_pPreviewNode,
      getFrameErrorMessage, sizeof(getFrameErrorMessage) - 1);
...
void VapourSynthScriptProcessor::colorAtPoint(size_t a_x, size_t a_y, double & a_rValue1,
...
void VapourSynthScriptProcessor::handleVSMessage(int a_messageType, const QString & a_message)
{
...
QPixmap VapourSynthScriptProcessor::pixmapFromFrame(const VSFrameRef * a_cpFrameRef)
{
  ...
  QImage frameImage(static_cast<const uchar *>(pData), width, height, stride, QImage::Format_RGB32);
...
bool VapourSynthScriptProcessor::initLibrary()
{
  ...
  Entry vssEntries[] = { { (QFunctionPointer *) &vssInit, "vsscript_init", "_vsscript_init@0" }, {
          ...
          "_vsscript_getCore@4" }, { (QFunctionPointer *) &vssGetOutput, "vsscript_getOutput",
          "_vsscript_getOutput@8" }, { (QFunctionPointer *) &vssFreeScript, "vsscript_freeScript",
          "_vsscript_freeScript@4" }, { (QFunctionPointer *) &vssFinalize, "vsscript_finalize",
...
double VapourSynthScriptProcessor::valueAtPoint(size_t a_x, size_t a_y, int a_plane)
{
  ...
  const uint8_t * cpPlane = m_cpVSAPI->getReadPtr(m_cpCurrentFrameRef, a_plane);
...
void VapourSynthScriptProcessor::initPreviewNode()
{
  ...
  switch (m_chromaResamplingFilter){
    ...
    case ResamplingFilter::Bicubic :
      resizeName = "Bicubic";
      m_cpVSAPI->propSetFloat(pArgumentMap, "filter_param_a_uv", m_resamplingFilterParameterA,
          paReplace);
      m_cpVSAPI->propSetFloat(pArgumentMap, "filter_param_b_uv", m_resamplingFilterParameterB,
          paReplace);
      break;
  ...
  VSMap * pResultMap = m_cpVSAPI->invoke(pResizePlugin, resizeName, pArgumentMap);
  ...
  VSNodeRef * pPreviewNode = m_cpVSAPI->propGetNode(pResultMap, "clip", 0, nullptr);
  assert(pPreviewNode);
